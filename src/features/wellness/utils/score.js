/* Readiness scoring engine — pure function.
   ...
   Manual-only inputs. Apple Health is gone; the score is built entirely
   from the morning check-in plus training cadence + habit signal.

   Weights:
     45%  sleep duration + quality
     30%  subjective energy + mood
     15%  muscular soreness
     10%  recent training load
     ±    flat bonuses/penalties from yesterday's habit completion
...
export function computeReadinessScore(inputs) {
  ...
  const effectiveSleep = sleepHours ?? 0;
  ...
  score += (sleepScore + sleepQualityMod) * 0.45;
